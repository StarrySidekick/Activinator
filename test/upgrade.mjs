...
(async () => {
  ...
  const three = await c.page.evaluate(() => ({
    ...
    packKept: ACT.S.packs.winter === true,
    newPackDefaulted: ACT.PACKS.filter(p => !(p.id in { core:1, winter:1 }))
      .every(p => ACT.S.packs[p.id] === p.on),
  ...
  const v3ok = out.v3.cards === 3 && out.v3.v === 4 && out.v3.editsEmpty && out.v3.packKept &&
    out.v3.newPackDefaulted && out.v3.ctxKept && out.v3.curates && !c.errs.length;
